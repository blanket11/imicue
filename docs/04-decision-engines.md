...
`jev-rubric-v2` で導入し、現在の `jev-rubric-v3` でも維持する条件として、lastSeenAgoMsが300,000ms以下のdirect観測だけをJevへ渡す。最低観測量もこの集合で再計算し、不足すればAPIを呼ばず `insufficient_evidence` とする。recentの個別イベントも300,000ms以下に限定する。時間比較はCoreで行い、モデルに判断させない。

30分の保存・集計期間と、既読・完了・閉じた候補の除外は変えない。観測の鮮度で絞る前に、期間内の記録全体で候補を除外する。Rulesは従来の5分半減期を維持する。

5分は未検証の暫定値で、連続的な時間減衰ではない。5分ちょうどは含み、1msでも超えると除外する。また、Snapshotはシグナル単位の集計なので、同じシグナルを再び観測すると過去30分の合計値も新しいlastSeenAgoMsとともに渡る。個別行動を厳密に5分間だけ再集計する仕様ではない。[実測と制約](15-freshness-and-live-browser.md)を参照する。
...
`jev-rubric-v3` は内容の関連性を次の4段階で評価する。明示操作が1回でも、内容が明確に一致するなら案内したいという方針を確認し、以前の段階2・3にあった根拠の重なりを整理した。

| 段階 | 判定基準 |
| --- | --- |
| 0 | 観測した内容との関連を示す材料がない、または無関係 |
| 1 | 同じ製品・大まかな分野に属するだけで、具体的なテーマや操作の一致を確認できない |
| 2 | 具体的なテーマは共通するが、観測した機能・操作そのものの説明ではなく、周辺の作業や背景情報を扱う |
| 3 | 観測した機能・操作・内容そのものを説明する。言い換えも含め、説明文から具体的な一致が分かる。明示操作は1回でもよい |

0は「人が興味を持っていない」ではなく「今回の入力から関連を支持できない」。表示時間だけで読了・好意を断定せず、未観測を否定的証拠にしないことをinstructionsに明記する。

最低観測量、鮮度、候補除外はコードで検査する。スコア・confidence・候補差の出力ゲートは変更しない。異なる採点基準の応答を混在させないため、v3のブラウザはv1・v2の応答を拒否する。サーバーとブラウザは一緒に更新する。
