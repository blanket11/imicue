...
`jev-rubric-v2` では、lastSeenAgoMsが300,000ms以下のdirect観測だけをJevへ渡す。最低観測量もこの集合で再計算し、不足すればAPIを呼ばず `insufficient_evidence` とする。recentの個別イベントも300,000ms以下に限定する。時間比較はCoreで行い、モデルに判断させない。
