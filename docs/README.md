...
M4のESM・型定義・IIFEとNext.js static export例も実装し、ローカルの静的配信から検証しました。M5の資料・評価レポート・梱包検査も追加しました。公開前の残る確認は [公開準備と評価](12-release-preparation.md) を参照してください。Vanillaの[ページ横断デモ](13-session-demo.md)も追加しました。[試す手順](../README.md#ページ移動後の記録を試す)を参照してください。Jev実API試験は未実施で、実装とは別に許可を受けて行います。
