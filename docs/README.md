...
**状態: v0.1 Draft / M0〜M2を実装、M3はモック検証まで完了**
...
このフォルダは実装者とCodexへの引き継ぎ用です。01〜08はv0.1全体の設計を含みます。現在の実装範囲と検証結果は [ローカル版の実装メモ](09-local-implementation.md) と [M3の実装メモ](10-server-implementation.md)、実行手順は [ルートREADME](../README.md) を参照してください。npmパッケージは未公開です。
...
Jev実API試験は未実施です。次の実装段階はM4の配布用ビルドとNext.js static export例、その後はM5の公開準備です。実API試験は実装とは別に許可を受けて行います。
