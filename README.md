...
同名の候補、複数の関心、関連候補がない場合などを試すには、上のコマンドに `JEV_EVALUATION_SUITE=semantic` を追加します。こちらは12件で最大12回のAPI呼び出しです。[推薦内容の確認表と実測結果](docs/17-recommendation-review.md)に、各場面の許容案と見送りの理由をまとめています。
